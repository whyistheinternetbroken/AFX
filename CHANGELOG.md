...
- **Primary post-create network verification/remediation.** After primary cluster creation, the script now verifies cluster-mgmt/node-mgmt LIF address+netmask+home-port and default route from the BMC/system-console shell, auto-fixing a mismatched `cluster_mgmt` home-port and creating a missing `0.0.0.0/0` route from config values.
