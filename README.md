...
| **Stale BMC Session Cleanup (5h)** | Interactive tool to list and clean up stale SSH/SOL connections to BMC/SP addresses. For diagnostics/cleanup, operators can target **all loaded BMCs** or a **single selected IP**. Includes an explicit `ipmitool sol deactivate`-only action plus full cleanup (drop in-process clients + ipmitool + optional stale-PID SIGTERM). Returns to main menu when done. |
