...
```bash
python3 reinit_afx_v2.py --config /path/to/myconfig.json
```
...
```bash
python3 reinit_afx_v2.py --config-example
```
...
Place `reinit_afx_v2.py` on the client machine that has network access to all BMC/SP addresses and the cluster management IP.

```bash
# Recommended: create a dedicated directory
mkdir ~/afx-reinit
cp reinit_afx_v2.py ~/afx-reinit/
cd ~/afx-reinit
```
...
# With debug output
python3 reinit_afx_v2.py --debug
...
nohup python3 reinit_afx_v2.py --bg --config configs/reinit-config.json > nohup.out 2>&1 &
```
...
4. Spawns: `screen -dmS afx-reinit python3 reinit_afx_v2.py --bg [other args]`
...
screen -S afx-reinit python3 reinit_afx_v2.py --bg --config configs/reinit-config.json
