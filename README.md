...
| **Experimental password groups for per-node BMC credentials** | In per-node credential flows, choosing **not** to use the same password now offers `Use password groups - EXPERIMENTAL? (y/n)`. You can define reusable password groups, assign nodes by numbered list, review a manifest, and restart grouping before proceeding. |
